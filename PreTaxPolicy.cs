                ...
                try
                {
                   decimal tax = 0;
                    ...
                    if (policy.Attributes.Contains("rev_country") && policy.Attributes.Contains("rev_firstname")&& policy.Attributes.Contains("rev_lastname")&& policy.Attributes.Contains("rev_drivingrecord")&& policy.Attributes.Contains("rev_numberofcars"))
                    {
                        ...
                        {
                            tracingService.Trace("trace for transaction " + policy.Attributes["transactioncurrencyid"].ToString());


                            Guid cadId = new Guid("9e6d3598-5f8f-e911-a9c1-000d3a33afd1");

                            ColumnSet cols = new ColumnSet(
                            new String[] { "currencyname" });

                            Entity currencyEntity = (Entity)service.Retrieve("transactioncurrency", cadId, cols);

                            tracingService.Trace("entity for currency entity" + currencyEntity.Attributes["currencyname"].ToString());

                            policy.Attributes["transactioncurrencyid"] = currencyEntity.ToEntityReference();
